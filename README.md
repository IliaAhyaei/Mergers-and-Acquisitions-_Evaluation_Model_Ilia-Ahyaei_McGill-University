# Mergers-and-Acquisitions-_Evaluation_Model_Ilia-Ahyaei_McGill-University
A clean, fully-automated M&amp;A accretion/dilution model in Excel. Includes deal financing, PPA &amp; goodwill, pro forma EPS, and accretion/dilution analysis.
M&A Accretion/Dilution Model (Excel)
A streamlined, fully automated M&A accretion/dilution model built in Excel.
This tool calculates:
Deal financing (cash / debt / stock mix)
After-tax cost of debt & weighted cost of acquisition
Purchase price allocation (PPA) & goodwill
Pro forma shares, net income, and EPS
Accretion/dilution %
Summary metrics for quick decision-making
Designed for students, case competitions, and interview preparation.

🧩 How to Use the Model

All inputs are located in the blue “Inputs” section on the left.

Update the following fields:

Purchase price

Premium (%)

Offer type (cash/stock/mixed)

Acquirer & target shares outstanding

Share prices

Exchange ratio

Cost of debt

Tax rate

Seller yield

Acquirer & target EPS

Synergies (pre-tax and after-tax)

Everything else in the spreadsheet updates automatically.

📈 Outputs You Get

The model computes:

Deal Financing

Cash / debt / stock allocation

After-tax cost of debt

Weighted cost of acquisition

Accretive or dilutive (Yes/No)


PPA & Goodwill

Fair value of net assets

Goodwill created

Identifiable intangibles

Liabilities assumed

Pro Forma EPS

Pro forma shares

Pro forma net income


Pro forma EPS

Accretion/dilution %

Summary Metrics

Accretive or dilutive

Total goodwill

Weighted cost of acquisition

Synergies included

🧪 Example Inputs (Optional Test Case)

Use these values to test the model:

Purchase Price: 500,000,000

Premium: 25%

Acquirer Shares: 200M

Target Shares: 40M

Acquirer Price: 25

Target Price: 10

Exchange Ratio: 0.4

Cost of Debt: 6%

Tax Rate: 25%

Acquirer EPS: 2.10

Target EPS: 1.20

After-tax Synergies: 22,500,000

Expected output:

Accretion ≈ +8%
